...
const brightness = ref(props.value ?? 80)

watch(() => props.value, (v) => { if (v != null) brightness.value = v })
watch(brightness, (v) => emit('update', v))
</script>
...
.prog-fill { height: 100%; border-radius: 3px; background: linear-gradient(90deg, #ffd54f, #ff9800); transition: width 0.3s ease; box-shadow: 0 0 8px var(--primary-glow); }
