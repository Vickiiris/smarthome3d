<template>
  ...
  <div class="prog-wrap">
    <div class="prog-bar"><div class="prog-fill speaker" :style="{ width: volume + '%' }"></div></div>
...
const volume = ref(props.value ?? 60)
watch(() => props.value, (v) => { if (v != null) volume.value = v })
watch(volume, (v) => emit('update', v))
</script>
...
.prog-fill.speaker { background: linear-gradient(90deg, #fb923c, #f97316); }
